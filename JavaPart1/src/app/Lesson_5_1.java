...
// Задача 11	
		...
		int sum11 = 0;
			for (int i=0; i<=n11.length-1; i++) {
				if (n11[i] == 0) {
					l11 = i;}	}
			System.out.println("Номер последнего " +l11);
			
			for (int i=0; i<=l11-1; i++) {
				if (n11[i] == 0) {
					f11 = i;}	}
			System.out.println("Номер первого " +f11);
			for (int i=f11; i<=l11; i++) {
				sum11 = sum11+n11[i];
			}
			System.out.println("Сумма " +sum11);
// Задача 12	
