...
//Задача 7
		//Дано целое число N (> 3). Если оно является степенью числа 3, то вывести True, 
...
System.out.println("Задача 14");	
	int x14 = 5; // дано
	int n14 = 3; // дано
	int s14 = 1; // для степени (-1)
	double d14 = x14; // окончательный ответ
	double e14 = 0; // знак + или -
		for (i=1; i<=n14; i++) {
			s14 = s14*(-1); //возводим в степень (-1)
			double xx14 = 1; // для вычисления степени
			double ff14 = 1; // для факториала
					for (j=1; j<=(2*i+1); j++) {
						xx14 = xx14*x14; // возводим в степень
						ff14 = ff14*j;	// факториал
						}
			e14 = xx14/ff14; // делим как в формуле
			e14 = e14*s14; //ставим знак числу - или+
			d14 = d14+e14; // складываем 1 и 2-е значение
